  ...
  return (
    <section className={classNames('', containerClassName)}>
      ...
      <div className="grid grid-cols-4 gap-x-4 gap-y-10">
        ...
        {/* {(mlMyIdeas?.myIdeas?.slice(0, loadMore) ?? [])
        ...
        {(mlMyIdeas?.myIdeas ?? [])
          ...
          .map((item, index) => {
            ...
            // const filterIdeasApprove = _.first(item?.filter(item => item.status === 'approve'));

            // console.log('filterIdeasApprove', filterIdeasApprove);
            // const filterIdeasApprove = item?.filter(itemApprove => itemApprove?.status === 'approve');

            return (
              <div key={index}>
                {/* <pre> {JSON.stringify(item?.status, null, ' ')} </pre> */}
                {/* {item?.status === 'approve' && (
                  <pre> {JSON.stringify(item?.status, null, ' ')} </pre>
                )} */}
